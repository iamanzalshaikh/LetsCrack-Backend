...
export const createOrUpdateQuestion = async (req, res, next) => {
    try {
        ...
        let question;
        if (module === 'writing') {
            question = await WritingQuestion.findOneAndUpdate({ testSetNumber: Number(testSetNumber), taskNumber: Number(taskNumber) }, { module: 'writing', testSetNumber: Number(testSetNumber), taskNumber: Number(taskNumber), ...content, updatedAt: new Date() }, { new: true, upsert: true });
        }
...
export const bulkImportQuestions = async (req, res, next) => {
    try {
        const { questions, dryRun = false } = req.body;
        ...
        const totalMatched = (writingResult?.matchedCount || 0) +
            (speakingResult?.matchedCount || 0) +
            (mcqResult?.matchedCount || 0);
        const totalModified = (writingResult?.modifiedCount || 0) +
 ...
 */
export const createOrUpdateTestSet = async (req, res, next) => {
    try {
        const { testSetNumber, title, description, modeSupport, modules, estimatedTimeMinutes, instructions, status, } = req.body;
        ...
        if (existing) {
            ...
            testSet = await TestSet.findOneAndUpdate({ testSetNumber: Number(testSetNumber) }, { $set: payload, $inc: { version: 1 } }, { new: true });
        }
        else {
 ...
 * Publish a test set
 */
export const publishTestSet = async (req, res, next) => {
    try {
 ...
 */
export const getTestSets = async (req, res, next) => {
    try {
 ...
 */
export const getRetentionReport = async (req, res, next) => {
    try {
        const now = new Date();
