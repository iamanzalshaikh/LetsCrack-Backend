 ...
 */
const gradeGenericRightPanel = (sections, studentAnswers) => {
    let correct = 0;
    let total = 0;
...
const resolveOptionText = (options, value) => {
    ...
    if (Array.isArray(options)) {
        ...
        if (!Number.isFinite(idx) || options[idx] === undefined)
            return String(value);
        return options[idx];
    }
    return String(value);
};
const READING_PART_TITLES = {
...
const buildReadingBreakdown = (bankTasks, answerDict) => {
    const breakdown = [];
    const sorted = [...bankTasks].sort((a, b) => Number(a.taskNumber || 0) - Number(b.taskNumber || 0));
    sorted.forEach((task, partIdx) => {
        const partNum = Number(task.taskNumber) || partIdx + 1;
        const partTitle = READING_PART_TITLES[partNum] ||
        ...
        for (const section of sections) {
            if (section.matchingQuestions && Array.isArray(section.matchingQuestions)) {
                for (const mq of section.matchingQuestions) {
                    ...
                    const isCorrect = yourAnswer !== undefined &&
...
const buildListeningBreakdown = (bankTasks, answerDict) => {
    const testSetNumber = bankTasks[0]?.testSetNumber || 1;
    const breakdown = [];
    const sorted = [...bankTasks].sort((a, b) => Number(a.taskNumber || 0) - Number(b.taskNumber || 0));
    sorted.forEach((task, partIdx) => {
        const partNum = Number(task.taskNumber) || partIdx + 1;
        const partTitle = LISTENING_PART_TITLES[partNum] ||
...
export const submitMcqAnswers = async (req, res, next) => {
    try {
        ...
        const { testSetNumber, module, taskNumber, answers } = req.body;
        ...
        const session = await TestSession.findOne({
            studentId,
            testSetNumber: Number(testSetNumber),
            status: modeForSubmit?.mode === 'practice'
        ...
        for (const task of bankTasks) {
            ...
            if (isGenericLayout && task.rightPanel) {
                ...
                task.rightPanel.sections.forEach((sec) => {
                    if (sec.questions) {
                        sec.questions.forEach((q) => {
                            const ansVal = answerDict[q.id];
                            const isCorr = ansVal !== undefined && (Array.isArray(q.correctAnswer) && Array.isArray(ansVal)
                    ...
                    if (sec.matchingQuestions) {
                        sec.matchingQuestions.forEach((mq) => {
                            const ansVal = answerDict[mq.id];
                            const isCorr = ansVal !== undefined && (String(mq.correctParagraph).trim().toLowerCase() === String(ansVal).trim().toLowerCase());
                            processedResponses.push({
                                module,
                                questionId: mq.id,
                                selectedOption: -1,
        ...
        if (scoreMapping && Array.isArray(scoreMapping.mappings) && scoreMapping.mappings.length > 0) {
            const match = scoreMapping.mappings.find((m) => correctCount >= m.minScore && correctCount <= m.maxScore);
            if (match)
                band = match.band;
        }
        else {
        ...
        if (selectedModules.includes('reading')) {
            ...
            submittedCount += nextResponses.filter((r) => r.module === 'reading').length;
        }
        if (selectedModules.includes('listening')) {
            ...
            submittedCount += nextResponses.filter((r) => r.module === 'listening').length;
        }
        ...
        const listeningTask = cache.listening[0];
        const listeningCount = listeningTask?.mcqs?.length || 0;
        const gradedReading = nextResponses.filter((r) => r.module === 'reading').length > 0 ? readingCount : 0;
        const gradedListening = nextResponses.filter((r) => r.module === 'listening').length > 0 ? listeningCount : 0;
        ...
        await result.save();
        const breakdown = module === 'reading'
...
export const getMcqTask = async (req, res, next) => {
    try {
        ...
        let task = tasksList.find((t) => !taskNumber || t.taskNumber === Number(taskNumber));
        ...
        if (task && !canViewAnswerKey) {
            ...
            if (task.mcqs) {
                task.mcqs = task.mcqs.map((m) => {
                    const { correctOption, ...rest } = m;
                    return rest;
                });
            }
            if (task.rightPanel?.sections) {
                task.rightPanel.sections = task.rightPanel.sections.map((sec) => {
                    if (sec.questions) {
                    ...
                    if (sec.matchingQuestions) {
                        sec.matchingQuestions = sec.matchingQuestions.map((q) => {
        ...
        res.json({
            ...
            answers: session.answers instanceof Map
                ? Object.fromEntries(session.answers.entries())
                : session.answers || {},
            reviewStatus: session.reviewStatus instanceof Map
 ...
 */
export const autosaveSessionState = async (req, res, next) => {
    try {
        const studentId = req.user.id;
