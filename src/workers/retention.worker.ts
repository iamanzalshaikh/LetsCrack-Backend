...
const deleteRawArtifacts = async (session: any) => {
  const audioUrls: string[] = (session.speakingRecordings || [])
    .map((r: any) => r.audioUrl)
    .filter(Boolean)
  ...
  session.writingResponses = (session.writingResponses || []).map((r: any) => ({
    ...(r.toObject?.() ?? r),
    responseText: undefined,
    aiAnalysis: undefined,
  }))
  session.speakingRecordings = (session.speakingRecordings || []).map((r: any) => ({
    ...(r.toObject?.() ?? r),
    audioUrl: undefined,
