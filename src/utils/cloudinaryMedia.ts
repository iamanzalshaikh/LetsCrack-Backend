 ...
 */
export async function uploadStudentSpeakingAudio(
  fileBuffer: Buffer,
  studentId: string,
  ...
  const part =
    taskNumber === 5 && (subTask === "A" || subTask === "B") ? `-${subTask.toLowerCase()}` : ""
 ...
 */
export async function tryDestroyCloudinaryUrl(url: string): Promise<void> {
...
function parseCloudinaryUrl(
  url: string
): { publicId: string; resourceType: "video" | "raw" | "image" } | null {
  try {
    const u = new URL(url)
    if (!u.hostname.includes("res.cloudinary.com")) return null
    const m = u.pathname.match(/\/(video|image|raw)\/upload\/(?:v\d+\/)?(.+)\.[a-zA-Z0-9]+$/)
    if (!m) return null
    return { resourceType: m[1] as "video" | "raw" | "image", publicId: decodeURIComponent(m[2]) }
  } catch {
    return null
  }
}
