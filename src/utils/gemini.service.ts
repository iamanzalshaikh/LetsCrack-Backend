...
const generateContentWithRetry = async (
  ...
  while (attempt < maxAttempts) {
    try {
      ...
      if (isDailyQuota) {
        logger.error(
          `Gemini API DAILY QUOTA EXHAUSTED. This API key has hit its daily request limit and cannot process more requests today. Please replace the GEMINI_API_KEY in .env with a fresh key from Google AI Studio.`
        );
      ...
      if (isTransient && attempt < maxAttempts) {
        ...
        try {
          const retryInfoDetail = error.errorDetails?.find?.((d: any) => d['@type']?.includes('RetryInfo'));
        ...
        logger.warn(
          `Gemini API Transient Error (${error.status || 'unknown'}). Retrying in ${sleepTime / 1000}s (Attempt ${attempt}/${maxAttempts})...`
        );
...
const clampCelpip6 = (value: unknown): number => {
  const n = Number(value);
  if (!Number.isFinite(n)) return 0;
  return Math.max(1, Math.min(6, Math.round(n)));
};

const safeStrList = (value: unknown, max: number) =>
  Array.isArray(value) ? value.slice(0, max).map((v) => String(v).trim()).filter(Boolean) : [];
...
const celpipWritingSharedRubric = (taskNumber: number) => `
...
export const buildBlendedCelpipWritingFeedback = (
  taskNumber: number,
  overall: number,
...
`;
    ...
    const resultText = result.response.text();
    const parsedResult = parseJsonSafely(resultText) as Record<string, unknown>;
    ...
    const rawBullets = (parsedResult?.categoryBullets || {}) as Record<string, unknown>;
    ...
    const lineFeedback = Array.isArray(parsedResult?.lineFeedback)
      ? (parsedResult.lineFeedback as unknown[])
          .slice(0, 6)
          .map((row: any) => ({
    ...
    const blendedFeedback = buildBlendedCelpipWritingFeedback(
      taskNumber,
      overall,
      overallRemark,
      categoryBullets,
