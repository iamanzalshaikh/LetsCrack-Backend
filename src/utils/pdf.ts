...
function coalesceAdjacentParagraphSegments(segments: DetailedSegment[]): DetailedSegment[] {
...
function collapseDuplicateRuleSegments(segments: DetailedSegment[]): DetailedSegment[] {
  const out: DetailedSegment[] = [];
  for (const seg of segments) {
    if (seg.type === 'rule' && out[out.length - 1]?.type === 'rule') continue;
    out.push(seg);
  }
  return out;
}
...
function clampPdfParagraph(raw: string, maxChars: number): string {
  const t = raw.trim();
...
function buildDetailedBody(task: AiReportTask): string {
  ...
  if (task.lineFeedback?.length) {
    ...
    const more =
      task.lineFeedback.length > MAX_LINE_ROWS
...
export const generateAiEvaluationReport = (res: Response, data: AiEvaluationReportData) => {
  ...
  const drawDetailedPages = (task: AiReportTask) => {
    ...
    let segmentsQueue: DetailedSegment[] = collapseDuplicateRuleSegments(
      coalesceAdjacentParagraphSegments(
        parseDetailedFeedbackMarkdown(detailedText).filter((seg) => {
          if (seg.type === 'p') return seg.text.trim().length > 0;
          return true;
        }),
      ),
    ...
    while (segmentsQueue.length > 0) {
      ...
      const slackNow = (): number => zoneBottom - yCursor - 2; // 6 -> 2

      const mustMoveToNextSheet = (): boolean => slackNow() < 8; // 12 -> 8
        ...
        if (head.type === 'h') {
          ...
          if (intrinsicH <= slab * 0.93) {
            ...
            const sliced = takeTextChunk(
              ...
              head.text,
              textW,
              Math.max(minHeadingSlab, slab - 6),
              fsz,
              2,
              'Helvetica-Bold',
            );
